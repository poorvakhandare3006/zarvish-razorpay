   ...
   <?php 
               ...
               if(isset($_GET['search'])){
                       ...
                       unset($_SESSION['sub_cat']);
                        ...
                        $sql = "SELECT COUNT(product_id) FROM products WHERE product_title LIKE '%$searchText%'";
                        ...
                        if($last != 1){
                          ...
                          if ($pagenum > 1) {
                                $previous = $pagenum - 1;
                            $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?page='.$previous.'&searchText='.$searchText.'&sort='.$sort.'&search=">Previous</a> &nbsp; &nbsp; ';
                            // Render clickable number links that should appear on the left of the target page number
                            for($i = $pagenum-4; $i < $pagenum; $i++){
                              if($i > 0){
                                    $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?page='.$i.'&searchText='.$searchText.'&sort='.$sort.'&search=">'.$i.'</a> &nbsp; ';
                              }
                              }
                            }
                          ...
                          // This does the same as above, only checking if we are on the last page, and then generating the "Next"
                            if ($pagenum != $last) {
                                $next = $pagenum + 1;
                                $paginationCtrls .= ' &nbsp; &nbsp; <a href="'.$_SERVER['PHP_SELF'].'?page='.$next.'&searchText='.$searchText.'&sort='.$sort.'&search=">Next</a> ';
                            }
                              ...
                              if ($pagenum > 1) {
                                    $previous = $pagenum - 1;
                                $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?page='.$previous.'&searchText='.$searchText.'&search=">Previous</a> &nbsp; &nbsp; ';
                                // Render clickable number links that should appear on the left of the target page number
                                for($i = $pagenum-4; $i < $pagenum; $i++){
                                  if($i > 0){
                                        $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?page='.$i.'&searchText='.$searchText.'&search=">'.$i.'</a> &nbsp; ';
                                  }
                                  }
                                }
                              // Render the target page number, but without it being a link
                              ...
                              // This does the same as above, only checking if we are on the last page, and then generating the "Next"
                                if ($pagenum != $last) {
                                    $next = $pagenum + 1;
                                    $paginationCtrls .= ' &nbsp; &nbsp; <a href="'.$_SERVER['PHP_SELF'].'?page='.$next.'&searchText='.$searchText.'&search=">Next</a> ';
                                }
                ...
                if (isset($_GET['sort'])) {
                    $sort = mysqli_real_escape_string($con,$_GET['sort']);
                    $query = "SELECT product_id,product_category_id,product_qty,product_title,product_actual_price,product_selling_price,product_label FROM products WHERE product_title LIKE '%$searchText%' AND main_product='yes' ORDER BY product_selling_price $sort $limit";

                } else {
                    $query = "SELECT product_id,product_category_id,product_qty,product_title,product_actual_price,product_selling_price,product_label FROM products WHERE product_title LIKE '%$searchText%' AND main_product='yes' ORDER BY product_id DESC $limit";
                }
            
                
                
                $run_search_query = mysqli_query($con,$query);
               ...
               if (mysqli_num_rows($run_search_query)<1) {
                ...
                while($row = mysqli_fetch_assoc($run_search_query)){
                    ...
                    $product_image_title = $row['product_image_title'];
                     ...
                     $list .= '<div class="col-6 col-md-4 col-lg-3">
                ...
                <div class="col-md-12 col-lg-9 col-xl-9">
                    <div class="content-indent container-fluid-custom-mobile-padding-02">
                        <div class="tt-filters-options">
                            <h1 class="tt-title">
                                <?php echo $searchText; ?><span class="tt-title-total">&nbsp;&nbsp;<?php echo $rows; ?></span>
                            </h1>
                            ...
                            <div class="tt-sort">
                                <form action="" method="GET">
                                    ...
                                    <input type="hidden" value="<?php echo $searchText; ?>" name="searchText">
